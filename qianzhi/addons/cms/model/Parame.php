 ...
 * 区块模型
 */
class Parame extends Model
{
    protected $name = "cms_parame";
    // 开启自动写入时间戳字段
    ...
    public static function getBlockList($params)
    {
        ...
        $orderway = empty($params['orderway']) ? 'asc' : strtolower($params['orderway']);
        ...
        foreach($list as $k => $lists){
            if(isset($lists['imagearr'])){
                $list[$k]['imagearr'] = explode(',',$lists['imagearr']);
                $list[$k]['imgdesc'] = explode(',',$lists['imgdesc']);
                // $intro = str_replace(array("[","]",'"'),'',$lists['intro']);
                // $list[$k]['intro'] = explode(',',$intro);
                // zjs新增
                $list[$k]['intro'] = json_decode($lists['intro'], true);
