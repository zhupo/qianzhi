...
class Archives extends Backend
{
    ...
    protected $noNeedRight = ['get_fields_html', 'check_element_available', 'suggestion'];
     ...
     * @param string $ids
     */
    public function copyArchives($ids = "")
    {
		//判断入参存不存在
        if ($ids) {
			//如果是多条数据则用foreach循环插入数据
            if(strstr($ids, ',')){
                $arr = explode(',', $ids);//字符串转数组
                foreach($arr as $k => $v){
					//查询要复制的数据
                    $info = Db::name('cms_archives')->where(['id'=>$v])->find();
                    if(!$info){
                        $this->error(__('The data no exist'));
                    }
					//给新数据赋值
                    $newData = $info;
                    unset($newData['id']);
                    ...
                    $newData['likes'] = 0;
                    $newData['dislikes'] = 0;
                    $newData['createtime'] = $newData['updatetime'] = $newData['publishtime'] = time();
                    if(!empty($newData['diyname'])){
                        $newData['diyname'] .= '-copy';
                    }
                    //执行插入
                    $res = Db::name('cms_archives')->insertGetId($newData);
                    if($res){
						//查询副表内容
                        $model = Db::name('cms_model')->where(['id'=>$info['model_id']])->field('table')->find();
                        $addonInfo = Db::name($model['table'])->where(['id'=>$v])->find();
                        if($addonInfo){
							//复制副表内容
                            $addonInfo['id'] = $res;
                            $addonRes = Db::name($model['table'])->insert($addonInfo, true);
                        }
                        
                    }else{
                        $this->error(__('Add failure'));
                    }
                }
                $this->success();
            }else{
				//查询要复制的数据
                $info = Db::name('cms_archives')->where(['id'=>$ids])->find();
                if(!$info){
                    $this->error(__('The data no exist'));
                }
				//给新数据赋值
                ...
                if($res){
					//查询副表内容
                    $model = Db::name('cms_model')->where(['id'=>$info['model_id']])->field('table')->find();
                    $addonInfo = Db::name($model['table'])->where(['id'=>$ids])->find();
                    if($addonInfo){
						//复制副表内容
                        $addonInfo['id'] = $res;
                        $addonRes = Db::name($model['table'])->insert($addonInfo, true);
                        //var_dump($addonRes);die();
                    }
                    $this->success();
                }else{
                    $this->error(__('Add failure'));
