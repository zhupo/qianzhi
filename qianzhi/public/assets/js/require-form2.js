    ...
    var Form = {
        ...
        api: {
            submit: function (form, success, error, submit) {
				...
				/*拼接图片描述字符串*/
				var v = '';
				$("input[name='desc[]']").each(function(i, j){
				...
				var addData = "&row[imgdesc]="+v;
				//console.log(addData);return false;
                //调用Ajax请求方法
                Fast.api.ajax({
                    type: type,
                    url: url,
                    data: form.serialize() + addData + (Object.keys(params).length > 0 ? '&' + $.param(params) : ''),
                    dataType: 'json',
