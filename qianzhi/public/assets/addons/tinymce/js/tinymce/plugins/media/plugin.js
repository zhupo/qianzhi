...
(function () {
    ...
    var getDefaultSize = function (editor) {
      return editor.getParam('media_default_size', {});
    };
    ...
    var dataToHtml = function (editor, dataIn) {
      ...
      if (data.embed) {
        ...
        var defaultSize = getDefaultSize(editor);
        if (typeof defaultSize == 'object' && Object.keys(defaultSize).length > 0) {
          data.width = defaultSize.width ? defaultSize.width : '';
          data.height = defaultSize.height ? defaultSize.height : '';
        } else {
    ...
    var createPreviewIframeNode = function (editor, node) {
      ...
      if (name === 'video') {
        var vd = node.firstChild.value;
        var b = /<source([\s]+|[\s]+[^<>]+[\s]+)src=(\"([^<>"\']*)\"|\'([^<>"\']*)\')[^<>]*>/gi;
        vd.match(b);
        var src_url = RegExp.$3;
        previewNode.attr({
          src: src_url,
